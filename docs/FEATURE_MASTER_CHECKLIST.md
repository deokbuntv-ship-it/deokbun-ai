...
| **미적용** | **2** | `CONSULTATION_INTELLIGENCE_DB` · `FORTUNE_MAIL_SETUP` — **의도적 제외.** ⚠ **2026-09-06 사유 정정**: "쓰는 코드가 아직 없어서" 라고 적었는데 **사실이 아니었다.** `admin/consultation-intelligence` 화면도 `adminIntelligenceService` 도 이미 있고, 그 서비스가 `admin_list_intelligence_runs`·`admin_get_intelligence_run` 을 부른다. 없는 것은 **그 표를 채우는 파이프라인**이다. 승격하면 빈 표를 읽는 화면이 생길 뿐이라 그대로 둔다. 대신 **메뉴에서 감췄다**(`AdminSidebar`, 화면 파일은 보존) — 오너에게 "열면 늘 비어 있는 메뉴" 로 보이던 것을 없앴다 |
...
| 6.14 | 자동 생성 → 게시 | **DONE** (2026-09-04) | `famous-suggest` Edge + `FamousSuggestion`(`basis: fact/interpretation/unknown` 으로 **날조 방지**) → 초안 → 게시. 파이프라인 존재 | ⚠ **2026-09-06 실태 조사**: LLM 이 만드는 것은 **한 줄 소개 · 3~5문장 소개 · SEO 제목/설명 · slug · 주제 아이디어**뿐이다. **본문(사주 해설)은 생성하지 않고, 프롬프트가 사주 계산을 명시적으로 금지**한다. 해석엔진도 연결돼 있지 않다(`famous_snapshots` insert 코드 0줄 · staging 0행). `docs/FAMOUS_SEO_SURVEY.md` §1-2·§1-3. ✅ **2026-09-04 해소** — `famous-compose` Edge 가 프로즌 엔진을 돌려 스냅샷을 저장하고(S1) 그 스냅샷만 근거로 **본문 5섹션**을 만든다(S3/S4). 금지 5종 + 반복 문두 + **근거 표기·자료 인용 누수·명식 모순**(v2) + **관계 대조·생극 방향·문장 결함**(v3) + **투간·통근·기둥·수치 대조·자기모순**(v4)을 **서버에서** 검사해 위반한 글은 초안으로도 내보내지 않는다. 세미콜론은 결정론적 clamp. **V6(2026-09-08)에서 "내용 없는 문장" 도 결정론적 clamp** — v1~v5 실측 356문장을 라벨러 3인이 전수 분류하고 규칙 후보 4개를 돌려, **정상 문장을 죽이지 않는 좁은 규칙 둘만** 남겼다(라벨러 합의 6건 중 2건만 잡고 4건은 의도적으로 놓친다). 84섹션 전수 재조립에서 섹션 첫 문장 적중 0 · 2문장 미만 섹션 0. 버린 문장은 응답(`droppedEmptySentences`)으로 보인다. staging 실측 3건 기계 위반 0 ⚠ **clamp 배포 직후 우회형 발생 — 이 계열은 미해결**(`FAMOUS_SAMPLE_OUTPUT_V6.md` §0). **V7(2026-09-08)에서 관계 검사기의 구멍 둘을 막았다** — ① 근거 괄호가 틀린 본문을 구제하던 것을 **본문만 대조**로, ② 한자 앵커가 없어 검사조차 안 되던 것을 **존재 서술 기준**으로. ⚠ **그동안의 "관계 N/N" 을 정정**: v4 는 7/7 이 아니라 11/11(4건 미검사), v6 은 9/9 가 아니라 **7/9**(2건 오류). 합성 반례 **정상 43 전부 통과 · 오류 20 전부 적발**. 응답에 **분모**(`relationMentions`)를 실었다. **V8(2026-09-08)에서 투간·통근의 분모도 바로잡았다** — "통근 29/29" 의 본문 기준은 **10/10**(v4 14/14→3/3 · v5 33/33→10/10). ⚠ 검사를 좁히지 **않았다**: 근거 인용도 독자가 읽고 그것을 쓰는 것은 LLM 이라 검사할 값이 있다(근거만 넣어도 투간 9/9·통근 77/77 일치, 구제 0건). 대신 `factClaimCounts` 로 **본문/근거를 따로** 낸다. 합성 반례 **정상 40 · 오류 20** 이 실측이 못 본 구멍 둘(투간 근거 구제 · 자리 묶어 쓰기)을 찾아 막았다. ⚠ 재생성이 **거짓 양성 3건**을 드러내 고쳤다(누적 29). 인물 1명 ≈ **9,307 토큰** |
...
| 9.8 | **변곡점 알림 (인앱)** | **DONE** | 2026-09-07. 대운 교체 · 세운 전환 · 두드러진 달을 인앱 알림으로. **새로 만든 것은 순수 함수 하나** — 테이블·RPC·화면·타입이 전부 이미 있었다. **LLM 0콜**(템플릿, 원가 0). 빈도 실측 6명 평균 **3.0회/년**. 톤 4종 금지를 회귀로 잠금 | ⚠ 앱을 열어야 생성됨(푸시는 계정 대기, `KNOWN_RISKS` M7). ⚠ 충·합 알림은 의도적 제외 |
| 9.9 | **원가 가시성 2차 — 오늘 얼마 · 마진 · 미리보기** | **DONE** | 2026-09-07. 집계 RPC 하나(`admin_ai_cost_window`)로 **대시보드 "오늘 AI 비용" · 저장 전 미리보기 · 캐시 정확도** 셋을 덮었다. **추론 토큰 단가 칸은 만들지 않았다** — 실측으로 출력에 포함됨을 확인(803건, 초과 0건). 마진은 덕 1개 값을 오너 입력으로 옮겨 냈고 **선불(매출 시점 ≠ 원가 시점)을 소비 시점 귀속으로** 푼다. 렌더 테스트 15개 추가(산술 ₩784 검증 포함) | ⚠ 마이그레이션 **파일만**. ⚠ request_type→상품 자동 매핑 없음(의도적) |
| 9.7 | **원가 가시성 — 단가를 데이터로** | **DONE** | 2026-09-06. 단가·환율을 `model_pricing`·`fx_rate` 표로 옮기고 관리자 표시 경로를 DB 전용으로(`adminPricingContract.test.ts` 가 코드 단가 사용을 금지). 모델별 원가에 **원화**가 붙고 **상품별 건당 원가**가 생겼다. staging 803건 산술 검증 — chat/mini **₩7**(문서 기준과 일치) | ⚠ 마이그레이션은 **파일만**(미적용). ⚠ 오너가 단가를 넣기 전까지 "가격 미확인". ⚠ 캐시 입력 컬럼 부재로 **+7% 과대 추정**. ⚠ 마진 %는 매출이 가설이라 내지 않음 |

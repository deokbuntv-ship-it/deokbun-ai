...
⚠ **정정 (2026-09-13) — 위 초안을 심사 노트로 쓰지 마십시오.**
- 2번은 **사실이 아닙니다.** 소비자 답변 화면에는 근거 표(천간·지지·십성)가 없습니다 — 근거 시트는 내부·관리자 화면입니다(`StructuredConsultationResult` 주석).
- 3번의 "대조 검사기 13종"은 **유명인 명식 페이지 본문 검사기**(`famousBodyPrompt.ts`)의 수치로, 상담 경로가 아닙니다.
- 사실 확인을 거친 제출용 문구는 `docs/STORE_LISTING_COPY_2026-09-13.md` §4 입니다(원본 `src/features/legal/storeListingCopy.ts` — 계약 테스트가 길이·금지 주장·가격을 지킵니다).

...
import { stripEngineLabels, stripPromptScaffolding } from '@/features/chat/presentation/commercialText';
...
// Presentation-boundary hygiene (V4 §8/§9/§69 + audit-driven remediation §11): strip any internal engine
// LABEL ("(엔진: SAJU)" …) or known prompt-scaffolding artifact ("(예시로 제시된 해)", stray "also") the model
// may have slipped in, from every user-facing string. Conservative — a no-op on a clean answer; never
// rewrites prose. The prompt is the primary defense; this is defense-in-depth at the render seam.
const clean = (s: string | undefined): string | undefined =>
  typeof s === 'string' ? stripPromptScaffolding(stripEngineLabels(s)) : s;
const cleanArr = (a: string[] | undefined): string[] | undefined => a?.map((x) => stripPromptScaffolding(stripEngineLabels(x)));

...
export type ChatServiceResult =
  ...
  | {
      ...
      errorCode:
        ...
        | 'AI_CONSENT_REQUIRED'
        | 'PROFILE_REQUIRED'
        // ⚠ 2026-09-17 — 서버가 **같은 요청을 아직 만들고 있다**(HTTP 409). 실패가 아니다.
        // 2026-09-15 실측: 30초에 끊긴 뒤 누른 "다시 시도" 가 이 409 를 받았는데 REQUEST_FAILED 로 뭉개져,
        // 화면에는 같은 실패 문구만 반복됐다. 화면은 이 코드를 받으면 기다렸다가 **같은 요청 번호로** 다시
        // 받아온다(새 질문을 보내면 LLM 을 한 번 더 태운다).
        | 'REQUEST_IN_PROGRESS';
      requestId?: string;

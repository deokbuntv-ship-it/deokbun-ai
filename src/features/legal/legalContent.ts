  ...
  sections: [
    {
      ...
      bullets: [
        ...
        '유입 경로(캠페인) 정보',
        // ⚠ 2026-09-12 추가 — 2026-09-11 묶음이 새로 모으기 시작한 것들. 코드 기준으로만 적는다.
        'AI 답변 신고 내용(신고한 답변의 식별자, 신고 사유, 이용자가 직접 적은 설명)',
        'AI 처리 동의 기록(동의한 문안의 버전, 동의·철회 시각, 동의한 환경(앱/웹))',
        '인앱 결제 기록(스토어 거래 식별자, 구매한 상품, 지급된 덕, 테스트 구매 여부, 결제 승인 처리 상태)',
        '공유 리포트 검증 정보(서비스가 만든 답변 문장의 해시값 — 문장 원문은 저장하지 않습니다)',
      ],
    },
    {
      heading: '2. 이용 목적',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        '[법률 검토] AI 해석 생성을 위해 OpenAI(미국)로 다음 항목이 전송됩니다: 생년월일과 태어난 시각, 성별, 태어난 지역, 이용자가 상담에서 입력한 질문과 대화 내용, 궁합 이용 시 상대방으로 등록한 사람의 생년월일시. 전송 목적은 이미 계산된 명식을 사람이 읽을 수 있는 해석문으로 바꾸는 것에 한정됩니다.',
        '[법률 검토] 위 전송에 대해서는 앱 또는 웹에서 별도의 명시적 동의를 받습니다. 동의는 버전과 동의 시각이 함께 기록되며, [MY] → [AI 처리 동의]에서 언제든 철회하실 수 있습니다. 철회하시면 그 시점부터 새로 전송하지 않으며, AI가 글을 만드는 기능(상담·오늘의 운세·이달의 운세·궁합·리포트)은 이용하실 수 없습니다.',
        '[법률 검토] 국외 이전에 관한 확정 기재(이전받는 자, 이전 국가·일시·방법, 이전 항목, 이용 목적, 보유 기간)는 최종 법률 검토 시 확정합니다.',
    ...
    },
    {
      ...
      paragraphs: [
        ...
        // ⚠ 2026-09-12 추가 — 항목별로 **지금 코드가 실제로 하는 일**을 적는다. 기간이 정해지지 않은
        //   것은 정해지지 않았다고 쓴다. 지키지 않는 기간을 약속하지 않는다.
        '[법률 검토] AI 답변 신고 내용은 서비스 품질 점검과 부적절한 답변 조치를 위해 보관하며, 회원 탈퇴 시 함께 삭제됩니다. 탈퇴 전 보관 기간의 상한은 최종 검토 시 정합니다.',
        '[법률 검토] AI 처리 동의 기록은 동의를 받았다는 사실을 증빙하기 위해 보관하며, 현재는 회원 탈퇴 시 함께 삭제됩니다. 탈퇴 후에도 증빙으로 보존해야 하는지는 최종 검토 시 정합니다.',
        '[법률 검토] 인앱 결제 기록은 회원 탈퇴 시 원래 형태는 삭제되지만, 전자상거래 관련 법령의 거래기록 보존을 위해 이용자를 알아볼 수 없는 형태(복원할 수 없는 해시로 된 식별자와 거래 요약)로 남깁니다. 보존 기간은 최종 검토 시 정합니다.',
        '공유 리포트 검증 정보는 문장 원문이 아닌 해시값이라 그 자체로 내용을 알 수 없으며, 회원 탈퇴 시 함께 삭제됩니다.',
      ],
    },
    {
      heading: '6. 이용자의 권리',
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        // ⚠ 2026-09-12 추가 — 안드로이드 결제가 붙었다. 서버가 실제로 하는 일(verify-purchase ·
        //   google-rtdn · record_revocation)을 그대로 적는다. 지키지 않는 약속을 하지 않는다.
        '[법률 검토] 덕 충전은 앱 마켓(Google Play · App Store)의 결제를 통해 이루어지며, 서비스는 앱 마켓에 결제 사실을 확인한 뒤에 덕을 지급합니다. 결제 확인이 끝나지 않은 결제에는 덕이 지급되지 않으며, 앱 마켓의 정책에 따라 자동으로 환불될 수 있습니다.',
        '[법률 검토] 앱 마켓을 통해 결제가 환불되거나 취소되면, 서비스는 그 결제로 지급된 덕을 회수합니다. 회수할 덕이 이미 사용되어 남아 있지 않은 경우 부족한 만큼은 이용자 계정의 미정산 금액으로 기록되며, 이후 충전되는 덕에서 먼저 차감됩니다. 보유 덕이 마이너스로 표시되지는 않습니다.',
        '[오너 결정 · 법률 검토 필요: 미성년자 결제의 법정대리인 동의와 취소권 — 부록 B 초안 참조]',
      ],
    },

-- ============================================================================
-- DeokbunAI — ADVERTISEMENTS & ACQUISITION (승격 묶음 5 / 5)
--
-- Consolidates docs/ADVERTISEMENTS_SETUP.sql (rev 2, production-schema-aligned): the ad/
-- acquisition tables, admin RLS, the three SERVER-TRUSTED conversion triggers, and the
-- admin_ad_performance RPC.
--
-- WHY THIS ONE IS DIFFERENT FROM THE OTHER FOUR BUNDLES
--   This is the only artifact that has NEVER been applied ANYWHERE. Staging never had it, and
--   production's first attempt on 2026-08-14 FAILED at a `set_updated_at()` dependency and left
--   nothing behind (docs/DATABASE_RUNBOOK.md §C). rev 2 fixed that root cause by defining its
--   own `ads_set_updated_at()` instead of depending on the shared one — so this is the first
--   successful application of the ad schema in any environment.
--
--   Everything below is copied VERBATIM from the rev-2 artifact except the surrounding
--   comments. The artifact was already fully idempotent (create-if-not-exists, or-replace,
--   drop-policy/trigger-if-exists) and contains no destructive statement, so rule 2 required no
--   rewrite here — the only `insert`s are inside trigger bodies and every one of them carries
--   `on conflict do nothing`.
--
-- WHAT STILL DOES NOT WORK AFTER THIS
--   Admin ad CRUD works immediately. The FUNNEL does not: the anonymous click and the
--   attribution anchor are written by the `ad-track` Edge Function, which is NOT deployed to
--   staging (PROJECT_STATE §3.1). Until it is, the 광고 성과 screen shows a truthful
--   "집계 준비 중" state — fail-closed, not a fake zero.
--
-- Depends on: public.is_admin() (묶음 1), auth.users, public.ai_usage_logs (20260902000000),
-- public.consultation_subjects (20260817000300). NO dependency on `profiles` or the shared
-- `set_updated_at()` — that is exactly what broke the first attempt.
-- ============================================================================
